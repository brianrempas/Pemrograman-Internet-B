    ...
    <div class="py-6">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="bg-white shadow-sm sm:rounded-lg p-4">
                ...
                <a href="{{ url('/') }}" class="px-4 py-2 bg-gray-500 text-white rounded hover:bg-gray-600">Kembali</a>
                <a href="{{ url('/prodi/create') }}"
                    class="px-4 py-2 bg-blue-600 text-white rounded hover:bg-blue-700">Tambah Prodi</a>
            </div>
                @else
                <div class="mb-4">
                    <a href="{{ url('/') }}"
                    ...
                    <table class="w-full border border-gray-200 divide-y divide-gray-200">
                        ...
                        <tbody class="bg-white divide-y divide-gray-200">
                            ...
                            <tr class="hover:bg-gray-50">
                                <td class="px-6 py-4 border-r border-gray-200">{{ $p->id }}</td>
                                <td class="px-6 py-4 border-r border-gray-200">{{ $p->nama_prodi }}</td>
                                <td class="px-6 py-4 border-r border-gray-200">{{ $p->fakultas->nama_fakultas ?? '-' }}</td>
                                @if(auth()->user()->role === 'admin')
                                <td class="px-6 py-4 text-center space-x-2">
                                    ...
                                    <form action="{{ url('/prodi/'.$p->id) }}" method="POST" class="inline"
