            ...
            <div class="bg-white shadow-sm sm:rounded-lg p-4">
                ...
                <div class="mb-4 flex justify-end space-x-2">
                    ...
                    <a href="{{ url('/mahasiswa/create') }}"
                        class="px-4 py-2 bg-blue-600 text-white rounded hover:bg-blue-700">Tambah Mahasiswa</a>
                </div>
                @else
                <div class="mb-4">
                    <a href="{{ url('/') }}"
                    ...
                    <table class="w-full border border-gray-200 divide-y divide-gray-200">
                        ...
                        <tbody class="bg-white divide-y divide-gray-200">
                            ...
                            <tr class="hover:bg-gray-50">
                                <td class="px-6 py-4 border-r border-gray-200">{{ $m->id }}</td>
                                <td class="px-6 py-4 border-r border-gray-200">{{ $m->nim }}</td>
                                <td class="px-6 py-4 border-r border-gray-200">{{ $m->nama }}</td>
                                <td class="px-6 py-4 border-r border-gray-200">{{ $m->prodi->nama_prodi ?? '-' }}</td>
                                <td class="px-6 py-4 border-r border-gray-200">
                                    {{ $m->prodi->fakultas->nama_fakultas ?? '-' }}</td>
                                @if(auth()->user()->role === 'admin')
                                <td class="px-6 py-4 text-center space-x-2">
                                    ...
                                    <form action="{{ url('/mahasiswa/'.$m->id) }}" method="POST" class="inline"
